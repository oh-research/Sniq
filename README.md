...
> 그리드 레이아웃 **두 개를 드래그 중 Flip 키로 뒤집어가며** 창을 배치하는 macOS 윈도우 스냅 유틸리티 — 다른 스냅 앱에는 없는 방식입니다. 드래그로 만든 배치는 **단축키로 저장**해 다음부터는 키 하나로 재현합니다.
...
1. **Ctrl (Grip)** 를 누른 채, 창 **어디든** 마우스로 잡고 드래그
